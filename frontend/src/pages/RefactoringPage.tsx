...
export const RefactoringPage = () => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
      {/* Header */}
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex-shrink-0 shadow-sm z-30 relative">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500">
            {modifiedGraph && (
              <>
                <span className="font-mono font-bold text-slate-700">{modifiedGraph.nodes.length}</span> nodes,{' '}
                <span className="font-mono font-bold text-slate-700">{modifiedGraph.edges.length}</span> edges
              </>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Left: Controls */}
        {controlsExpanded ? (
          <div className="w-72 border-r border-slate-200 bg-white overflow-y-auto p-4 custom-scrollbar z-20 flex-shrink-0">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wide">Graph Modifications</h3>
              <button onClick={() => setControlsExpanded(false)} className="text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded p-1">
            ...
            {changes.length > 0 && (
              <div className="mt-4 pt-4 border-t border-slate-200">
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Change History</h3>
                <div className="space-y-2">
                  {changes.slice().reverse().map((change, idx) => (
                    <div
                      key={change.id}
                      className="text-xs bg-slate-50 border border-slate-200 rounded-md p-2 group transition-colors hover:border-teal-300 hover:bg-teal-50/30 cursor-pointer"
                      onClick={() => {
                        if (change.type === 'add_node' && change.data?.nodeId) {
                          // Use a timestamp to force update even if clicking the same node
                          setFocusNodeId(null);
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          <div className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold uppercase mb-1 ${change.type.includes('remove') ? 'bg-rose-100 text-rose-700' : 'bg-teal-100 text-teal-700'
                            }`}>
        ...
        <div className="flex-1 relative bg-slate-50 overflow-hidden"
          ...
          */}
          ...
          {/* Empty State Overlay */}
          {changes.length === 0 && modifiedGraph && (
            <div className="absolute top-4 left-4 max-w-xs z-10 pointer-events-none">
              <div className="bg-blue-50/90 backdrop-blur border border-blue-200 rounded-lg p-3 shadow-sm">
                <div className="flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-blue-600 flex-shrink-0 mt-0.5" />
                  <div className="flex-1 text-xs text-blue-900">
                    <p className="font-semibold mb-1">What-If Mode</p>
                    <p className="text-blue-800">
                      Use controls on the left to modify the graph and see changes in real-time
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="w-96 border-l border-slate-200 bg-white flex flex-col z-20 flex-shrink-0 overflow-hidden">
          ...
          {/* Content - Grows to fill space, handles its own scroll */}
