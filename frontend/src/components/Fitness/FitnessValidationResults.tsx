    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-md ${result.passed ? 'bg-teal-50 text-teal-600' : 'bg-rose-50 text-rose-600'}`}>
            {result.passed ? <CheckCircle className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
          </div>
          <div>
            <h3 className={`text-sm font-bold ${result.passed ? 'text-teal-800' : 'text-rose-800'}`}>
        ...
        <div className="flex flex-wrap gap-1.5">
          <StatBadge label="rules" value={result.total_rules} color="bg-slate-100 text-slate-700" />
          {result.errors > 0 && <StatBadge label="errors" value={result.errors} color="bg-rose-100 text-rose-700" />}
          {result.warnings > 0 && <StatBadge label="warns" value={result.warnings} color="bg-amber-100 text-amber-700" />}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded border ${severityStyles[violation.severity]}`}>
                          {violation.severity}
                        </span>
                      ...
                      {violation.affected_modules.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {violation.affected_modules.slice(0, 4).map((module, idx) => (
                            <span key={idx} className="text-xs font-mono text-slate-600 bg-white border border-slate-200 px-1.5 py-0.5 rounded">
                              {module}
                            </span>
                          ))}
...
const StatBadge = ({ label, value, color }: { label: string, value: number, color: string }) => (
  <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium ${color}`}>
