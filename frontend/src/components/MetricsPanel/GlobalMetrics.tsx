    ...
    <div className="p-3 md:p-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-surface border border-border-light rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div className="text-center">
              <div className="text-2xl font-extrabold text-primary tabular-nums">{globalMetrics.total_files}</div>
              <div className="text-xs text-text-secondary mt-1 font-medium">Total</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-extrabold text-success tabular-nums">{globalMetrics.total_internal}</div>
              <div className="text-xs text-text-secondary mt-1 font-medium">Internal</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-extrabold text-info tabular-nums">{globalMetrics.total_third_party}</div>
        ...
        <div className="bg-surface border border-border-light rounded-lg p-3 relative shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between items-center">
              <span className="text-text-secondary text-xs">Avg Afferent:</span>
              <span className="font-mono font-bold text-text-primary tabular-nums text-base">{globalMetrics.avg_afferent_coupling}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-text-secondary text-xs">Avg Efferent:</span>
              <span className="font-mono font-bold text-text-primary tabular-nums text-base">{globalMetrics.avg_efferent_coupling}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-text-secondary text-xs">Threshold:</span>
              <span className="font-mono font-bold text-text-primary tabular-nums text-base">{globalMetrics.coupling_threshold}</span>
            </div>
          </div>
        </div>
            ...
            <div className="max-h-20 overflow-y-auto pr-1 custom-scrollbar">
              <ul className="text-xs space-y-1 text-warning/90">
                {globalMetrics.high_coupling_files.map((file, idx) => (
                  <li key={idx} className="truncate font-mono bg-surface px-2 py-1 rounded border border-warning/20">
                    • {file}
                  </li>
                ))}
              </ul>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-text-secondary text-xs">Avg Complexity:</span>
              </div>
              <span className="font-mono font-bold text-text-primary tabular-nums text-base">{globalMetrics.avg_complexity}</span>
            </div>
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-green-500" />
                <span className="text-text-secondary text-xs">Avg Maintainability:</span>
              </div>
              <span className="font-mono font-bold text-text-primary tabular-nums text-base">{globalMetrics.avg_maintainability}</span>
            </div>
          </div>
        </div>
        ...
        {globalMetrics.circular_dependencies.length > 0 && (
          <div className="bg-error-bg border border-error/30 rounded-lg p-3 md:col-span-2 lg:col-span-3 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="max-h-20 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
              {globalMetrics.circular_dependencies.map((dep, idx) => (
                <div key={idx} className="text-xs bg-surface border border-error/20 p-2 rounded font-mono text-error/90">
                  {dep.cycle.join(' → ')}
                </div>
              ))}
            </div>
        ...
        {globalMetrics.hot_zone_files && globalMetrics.hot_zone_files.length > 0 && (
          <div className="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-300/50 rounded-lg p-3 md:col-span-2 lg:col-span-3 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="max-h-32 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {globalMetrics.hot_zone_files.map((file, idx) => (
                <div key={idx} className={`text-xs bg-white/80 border p-2.5 rounded font-mono ${
                  ...
                  <div className="text-xs text-gray-600 mb-1">{file.reason}</div>
