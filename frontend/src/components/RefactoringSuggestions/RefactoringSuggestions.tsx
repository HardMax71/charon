            ...
            {Object.entries(summary.by_pattern).map(([pattern, count]) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <PatternIcon pattern={pattern} className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-amber-600' : 'text-slate-400'}`} />
                  <div className="min-w-0 flex-1">
                    <div className={`text-[10px] leading-tight truncate ${isActive ? 'text-amber-700' : 'text-slate-500'}`}>{pattern}</div>
        ...
        {Array.from(suggestionsByPattern.entries()).map(([pattern, patternSuggestions]) => (
          <div key={pattern} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex items-center gap-2">
              <PatternIcon pattern={pattern} className="w-3.5 h-3.5 text-slate-400" />
                  ...
                  <div key={globalIdx}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[11px] text-slate-400 truncate flex-1">{suggestion.module}</span>
                        ...
                        <div className="border-t border-slate-100 pt-3 space-y-3">
                          ...
                          <div className="bg-white border border-slate-200 rounded-lg p-3">
                            ...
                            <div className="flex items-start gap-2 pt-2 border-t border-slate-100">
                              ...
                              <p className="text-xs text-slate-600">{suggestion.suggested_refactoring}</p>
                            </div>
                          </div>
                          ...
                          {Object.keys(suggestion.metrics).length > 0 && (
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-[10px] text-slate-400 uppercase font-medium">Metrics:</span>
                              {Object.entries(suggestion.metrics).map(([key, val]) => (
                                <span key={key} className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 rounded text-[11px]">
                                  <span className="text-slate-500">{key.split('_')[0]}</span>
